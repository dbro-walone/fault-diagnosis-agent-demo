...
for fingerprint in fingerprints:
    for ref in fingerprint.get("matched_log_ids", []):
        if ref not in log_ids:
            errors.append(f"{fingerprint.get('fingerprint_id')} unknown matched log: {ref}")

for series in kpis:
    timestamps = [point.get("timestamp") for point in series.get("points", [])]
    if timestamps != sorted(timestamps):
        errors.append(f"{series.get('series_id')} KPI points are not time ordered")
...
for field in ("root_cause_chain", "impact_chain", "recovery_chain"):
    for ref in conclusion.get(field, []):
        if ref not in resource_ids:
            errors.append(f"conclusion {field} unknown resource: {ref}")

for ref in conclusion.get("key_evidence_refs", []):
    if ref not in evidence_ids:
        errors.append(f"conclusion unknown key evidence: {ref}")

known_task_results = resource_ids | edge_ids | symptom_ids | alarm_ids | log_ids | fingerprint_ids | kpi_ids | similar_ids
for task in tasks:
    for ref in task.get("result_refs", []):
        if ref not in known_task_results:
            errors.append(f"{task.get('task_id')} unknown result_ref: {ref}")

diagnosis_id = conclusion.get("diagnosis_id")
known_story_data = source_ids | symptom_ids | {diagnosis_id, "repair"}
for scene in storyboard:
    for ref in scene.get("focus_resource_ids", []):
        if ref not in resource_ids:
            errors.append(f"{scene.get('scene_id')} unknown focus resource: {ref}")
    for ref in scene.get("candidate_ids", []):
        if ref not in candidate_ids:
            errors.append(f"{scene.get('scene_id')} unknown candidate: {ref}")
    for ref in scene.get("task_ids", []):
        if ref not in task_ids:
            errors.append(f"{scene.get('scene_id')} unknown task: {ref}")
    for ref in scene.get("evidence_ids", []):
        if ref not in evidence_ids:
            errors.append(f"{scene.get('scene_id')} unknown evidence: {ref}")
    for ref in scene.get("data_refs", []):
        if ref not in known_story_data:
            errors.append(f"{scene.get('scene_id')} unknown data_ref: {ref}")

if manifest.get("case_id") != case.get("case_id"):
    errors.append("manifest and case.json case_id mismatch")
if case.get("case_id") and root.resolve().name != case.get("case_id"):
    errors.append(
        f"case directory name mismatch: {root.resolve().name} != {case.get('case_id')}"
    )

trigger = case.get("trigger", {})
if trigger.get("object_id") not in resource_ids:
    errors.append("case trigger object does not exist")
if trigger.get("symptom_id") not in symptom_ids:
    errors.append("case trigger symptom does not exist")
