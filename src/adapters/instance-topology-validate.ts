 ...
 * 保证运行时与编译快照同一套校验口径。
 *
 * §5.11 规则编号（错误码前缀见 docs/19 §17.2）：
 *  1  resource_type_code 可映射 KG L1；                          → IT-KG-001
 *  2  关系端点 / 状态主体 / RelationSet 成员全部存在；           → IT-REF-001
 *  3  每条实例关系和事件端点均有 L1 类型能力支持；              → IT-KG-002
 *  4  CONTAINS 有向无环 + 一个资源最多一个直接容器；             → IT-SEM-001
 *  5  DEVICE_INTERNAL 资源必须存在 Storage Device 祖先；         → IT-SEM-002
 *  6  外部资源不得直接连接设备内部非边界资源（仅物理 CONNECTS_TO）；→ IT-SEM-003
 *  7  对称关系规范排序且不得重复；                              → IT-SEM-004
 *  8  状态时间不超出主体生命周期，同维度时间不重叠；             → IT-TIME-001
 *  9  Snapshot 不包含 snapshot_at 之后才成立的状态；             → IT-TIME-002
 *  10 FAILOVER_TO、AFFECTS 禁止进入稳定关系集合；               → IT-STATE-001
 *  11 资源和关系禁止携带候选、根因、证据和最终影响结论；         → IT-STATE-002
 *  12 基础拓扑禁止携带坐标、颜色、光晕、展开状态和 Storyboard 幕次；→ IT-STATE-003
 */
...
export function validateInstanceTopology(
  ...
  for (const r of snapshot.resources) {
    if (!rtCodes.has(r.resource_type_code)) {
      fail('IT-KG-001', `资源 ${r.resource_id} resource_type_code=${r.resource_type_code} 无法映射 KG L1 ResourceType`)
    }
  }

  // §5.11-2 端点 / 状态主体 / 成员存在。
  for (const rel of snapshot.relations) {
    if (!resourcesById.has(rel.source_ref)) fail('IT-REF-001', `关系 ${rel.relation_id} 源端点悬空：${rel.source_ref}`)
    if (!resourcesById.has(rel.target_ref)) fail('IT-REF-001', `关系 ${rel.relation_id} 目标端点悬空：${rel.target_ref}`)
  }
  for (const st of snapshot.states) {
    ...
    if (!exists) fail('IT-REF-001', `状态 ${st.state_id} 主体悬空：${st.subject_kind}:${st.subject_ref}`)
  }
  for (const set of snapshot.relation_sets) {
    for (const m of set.members) {
      if (!resourcesById.has(m.member_ref) && !relationsById.has(m.member_ref) && !setsById.has(m.member_ref)) {
        fail('IT-REF-001', `RelationSet ${set.relation_set_id} 成员悬空：${m.member_ref}`)
      }
    }
  }
  ...
  for (const rel of snapshot.relations) {
    ...
    if (!cap) {
      fail('IT-KG-002', `关系类型 ${rel.relation_type} 无 L1 类型能力（topology_capabilities）`)
      continue
    }
  ...
  for (const ev of snapshot.events) {
    ...
    if (!cap) {
      fail('IT-KG-002', `事件类型 TopologyEvent.${ev.event_type} 无 L1 类型能力`)
      continue
    }
    ...
    if (!cap.source.has('*') && !cap.source.has(src.resource_type_code)) {
      fail('IT-KG-002', `事件 ${ev.event_id} 源端点类型能力不符：${src.resource_type_code}`)
    }
    if (!cap.target.has('*') && !cap.target.has(tgt.resource_type_code)) {
      fail('IT-KG-002', `事件 ${ev.event_id} 目标端点类型能力不符：${tgt.resource_type_code}`)
    }
  }
  ...
  for (const rel of snapshot.relations) {
    ...
    if (prev !== undefined && prev !== rel.source_ref) {
      fail('IT-SEM-001', `资源 ${rel.target_ref} 有多个直接容器：${prev} 与 ${rel.source_ref}`)
    }
  ...
  for (const child of containerOf.keys()) {
    ...
    while (containerOf.has(cur) && guard++ < 1000) {
      if (seen.has(cur)) {
        fail('IT-SEM-001', `CONTAINS 形成环：${[...seen, cur].join(' → ')}`)
        break
      }
      seen.add(cur)
  ...
  for (const r of snapshot.resources) {
    ...
    if (!hasStorageAncestor) {
      fail('IT-SEM-002', `DEVICE_INTERNAL 资源 ${r.resource_id} 缺少 Storage Device 祖先`)
    }
  }
  ...
  for (const rel of snapshot.relations) {
    if (SYMMETRIC_RELATIONS.has(rel.relation_type)) {
      if (rel.source_ref > rel.target_ref) {
        fail('IT-SEM-004', `对称关系 ${rel.relation_id} (${rel.relation_type}) 未按 resource_id 规范排序`)
      }
    }
    const key = `${rel.relation_type}|${rel.source_ref}|${rel.target_ref}`
    if (seenRelations.has(key)) {
      fail('IT-SEM-004', `重复稳定关系：${rel.relation_id}（${key}）`)
    }
  ...
  for (const st of snapshot.states) {
    if (st.subject_kind === 'RESOURCE') {
      ...
      if (res && st.valid_time.from && res.valid_time.to && st.valid_time.from > res.valid_time.to) {
        fail('IT-TIME-001', `状态 ${st.state_id} 起始晚于主体生命周期结束`)
      }
    }
  }
  ...
  for (const bucket of stateBuckets.values()) {
    ...
    for (let i = 0; i < timed.length; i++) {
      for (let j = i + 1; j < timed.length; j++) {
        ...
        if (overlap) {
          fail('IT-TIME-001', `状态 ${a.state_id} 与 ${b.state_id} 同维度有效时间重叠`)
        }
      }
    }
  }
  ...
  if (snapshot.snapshot_at) {
    for (const st of snapshot.states) {
      if (st.valid_time.from && st.valid_time.from > snapshot.snapshot_at) {
        fail('IT-TIME-002', `状态 ${st.state_id} 起始 ${st.valid_time.from} 晚于 snapshot_at ${snapshot.snapshot_at}`)
      }
    }
  }
  ...
  for (const rel of snapshot.relations) {
    if (rel.relation_type === 'FAILOVER_TO' || rel.relation_type === 'AFFECTS') {
      fail('IT-STATE-001', `稳定关系集合禁止出现 ${rel.relation_type}：${rel.relation_id}`)
    }
  }
  ...
  for (const r of snapshot.resources) {
    ...
    if (clash.length) fail('IT-STATE-002', `资源 ${r.resource_id} 携带运行时诊断语义字段：${clash.join(',')}`)
  }
  for (const rel of snapshot.relations) {
    const clash = Object.keys(rel.properties).filter((k) =>
      FORBIDDEN_RUNTIME_KEYS.some((f) => k.toLowerCase().includes(f)),
    )
    if (clash.length) fail('IT-STATE-002', `关系 ${rel.relation_id} 携带运行时诊断语义字段：${clash.join(',')}`)
  }

  // §5.11-12 基础拓扑禁止携带坐标、颜色、光晕、展开状态和 Storyboard 幕次。
  for (const r of snapshot.resources) {
    const clash = Object.keys(r.properties).filter((k) => FORBIDDEN_VIEW_KEYS.includes(k))
    if (clash.length) fail('IT-STATE-003', `资源 ${r.resource_id} 携带投影字段：${clash.join(',')}`)
  }
  for (const rel of snapshot.relations) {
    const clash = Object.keys(rel.properties).filter((k) => FORBIDDEN_VIEW_KEYS.includes(k))
    if (clash.length) fail('IT-STATE-003', `关系 ${rel.relation_id} 携带投影字段：${clash.join(',')}`)
  }
  ...
  for (const r of snapshot.resources) {
    ...
    if (attrs && Object.prototype.hasOwnProperty.call(attrs, 'display')) {
      warn('IT-SEM-WARN-DISPLAY', `资源 ${r.resource_id} attributes.display 应拆分到 View Projection Hint（§9.3）`)
    }
  }
